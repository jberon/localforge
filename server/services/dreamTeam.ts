...
- Aria (Product Architect): System design, architecture, planning
- Forge (Senior Engineer): React/TypeScript implementation
- Pixel (UX Designer): Interface design, accessibility
...
const BUSINESS_CASE_PROMPT = `You are Aria, the Product Architect. Analyze the user's app request and create a comprehensive business case.
...
Create a business case that will guide the development team. Be specific and actionable.
...
RESPOND WITH JSON ONLY:
{
  ...
  "valueProposition": "Why would someone use this?",
...
const README_PROMPT = `You are Forge, the Senior Engineer. Generate a professional README.md for this project.
...
export class DreamTeamService {
  ...
  async analyzeAndCreateSpecialists(
    ...
    const aria = this.getMemberById("aria")!;
  ...
  async generateBusinessCase(
    ...
    try {
      const response = await streamCompletion(
        ...
        {
          systemPrompt: "You are Aria, the Product Architect.",
          messages: [{ role: "user", content: prompt }],
          maxTokens: 3000,
          onChunk: onThinking,
        }
  ...
  async generateReadme(
    ...
    const forge = this.getMemberById("forge")!;
    ...
    try {
      const content = await streamCompletion(
        ...
        {
          systemPrompt: "You are Forge, the Senior Engineer.",
          messages: [{ role: "user", content: prompt }],
          maxTokens: 2000,
          onChunk: onThinking,
        }
